...
;;; Updated: Fri Jul  7 19:55:29 1995 by Jeff Dalton
;;; Release Version: %Y%
...
(defmessage (:world :always) (pv-pairs)
  (loop for (pattern value) in pv-pairs
	do (set-always-value pattern value)))
...
(defun plan-step-real-success (step)
  ...
  (loop for (pattern value) in (plan-step-effects step)
	unless (has-always-value-p pattern)
	do (set-pattern-value pattern value))
  ;; Tell the Exec
  (report-to-exec
     :success
     (current-simulated-time)
...
(defun plan-step-failure (step reason &optional (failed-effects nil partial-p))
  ...
  (when partial-p
    (loop for e in (plan-step-effects step)
	  for (p v) = e
	  unless (or (has-always-value-p p)
		     (member e failed-effects :test #'equal))
	  do (set-pattern-value p v)))
  ;; Tell the Exec
  (report-to-exec
     :failure
     (current-simulated-time)
...
(defun report-to-exec (&rest agenda-body)
  (ipc-write :worldout agenda-body))
  ...
  (if (null effects)
    ...
    (let ((always-conflicts
	   (loop for e in effects
		 for (p v) = e
		 when (and (has-always-value-p p)
			   (not (always-p p v)))
		 collect e))
	  (fact-conflicts
	   (loop for (p v) in effects
		 when (and (pattern-has-value-p p)
			   (not (fact-p p v)))
		 collect `(,p ,(get-pattern-value p) ,v))))
...
(definit :world *always-table* (make-hash-table :test #'equal))

(defun set-always-value (pattern value)
  (setf (gethash pattern *always-table*) value))

(defun get-always-value (pattern)
  (gethash pattern *always-table*))

(defun has-always-value-p (pattern)
  (multiple-value-bind (value there-p) (get-always-value pattern)
    (values there-p value)))

(defun always-p (pattern desired-value)
  (multiple-value-bind (value there-p) (get-always-value pattern)
    (if there-p
	(equal value desired-value)
      nil)))

(defun display-always (&optional (stream *interact*))
  (let ((*package* (find-package :oplan))) ;get pkg prefixes right in output
    (loop for (pat . val)
	  in (canonical-description-order
	      (hash-table-alist *always-table*))
