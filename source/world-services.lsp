...
;;; Updated: Thu Jun  1 00:15:38 1995 by Jeff Dalton
;;; Release Version: %Y%
...
(defun send-to-exec (&rest agenda-body)
  (ipc-send :rightin :world-event agenda-body))
...
(defun set-pattern-value (pattern value)
  (setf (gethash pattern *world-state*) value))

(defun get-pattern-value (pattern)
  (gethash pattern *world-state*))

(defun pattern-has-value-p (pattern)
  (multiple-value-bind (value there-p) (get-pattern-value pattern)
    (values there-p value)))

(defun fact-p (pattern desired-value)
  (multiple-value-bind (value there-p) (get-pattern-value pattern)
    (if there-p
